...
public class Trajetos implements Serializable {
	...
	static public void lerArquivo() {
		...
		try {
			FileReader fr = new FileReader("Trajetos.json");
		...
		for (int i = 0; i < json.length(); i++) {
			...
			for (int j = 0; j < trechos.length(); j++) {
				Trecho n = Trechos.findTrecho(trechos.getString(i));
				if(n == null) {
	...
	static public void salvarArquivo() {
		...
		try {
			FileWriter fw = new FileWriter("Trajetos.json");
	...
	static public void preencherTrajeto(String idTrajeto, String idTrecho) {
		Trecho t = Trechos.findTrecho(idTrecho); 
		if(t == null) {
