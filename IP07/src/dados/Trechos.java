...
public class Trechos implements Serializable {
	...
	static public void lerArquivo() {
		...
		try {
			FileReader fr = new FileReader("Trechos.json");
		...
		for (int i = 0; i < json.length(); i++) {
			...
			Ponto a = Pontos.findPonto(json.getJSONObject(i).getString("partida"));
			Ponto b = Pontos.findPonto(json.getJSONObject(i).getString("destino"));
	...
	static public void salvarArquivo() {
		...
		try {
			FileWriter fw = new FileWriter("Trechos.json");
	...
	static public void addTrecho(String origem, String destino) {
		Ponto a = Pontos.findPonto(origem);
		Ponto b = Pontos.findPonto(destino);
	...
	static public void remTrecho(String id) {
		...
		Trajetos.remTrajetosByTrecho(remover);
